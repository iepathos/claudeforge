...
async fn test_is_dir_empty_with_subdirectory() {
    ...
    fs::create_dir_all(dir_with_subdir.join("subdir")).await.unwrap();
...
async fn test_copy_dir_recursive_handles_symlinks() {
    ...
    #[cfg(unix)]
    {
        ...
        fs::write(src_dir.join("file.txt"), "content").await.unwrap();
...
async fn test_copy_dir_recursive_with_nested_exclusions() {
    ...
    fs::write(excluded.join("package.json"), "{}").await.unwrap();
