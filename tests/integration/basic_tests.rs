    ...
    for (language, template) in registry.iter() {
        ...
        match result {
            Ok(_) => {
                ...
                assert!(project_dir.exists(), "Project directory should exist for {} template", template.name);
            }
            Err(e) => {
                // If template repositories don't exist, this is expected
                println!("Expected error for {} template (repo might not exist): {e}", template.name);
            }
        }
    }
}
